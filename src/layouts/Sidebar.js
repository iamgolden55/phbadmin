...
// Wrapper component to inject auth into Sidebar
function SidebarWithAuth(props) {
    const { hasPermission } = useAuth();
    return <SidebarInner hasPermission={hasPermission} {...props} />;
}
        ...
        return (
            <div className="sidebar">
                ...
                <PerfectScrollbar className="sidebar-body" ref={ref => this._scrollBarRef = ref}>
                    <SidebarMenu hasPermission={this.props.hasPermission} onUpdateSize={() => this._scrollBarRef.updateScroll()} />
                </PerfectScrollbar>
    ...
    populateMenu = (m) => {
        ...
        const menu = m.map((m, key) => {
            ...
            if (m.submenu) {
                ...
                const filteredSubmenu = m.submenu.filter(item => {
                    // If no permission required, show the item
                    if (!item.permission) return true;
                    // If hasPermission function is available, check permission
                    if (hasPermission) return hasPermission(item.permission);
                    // Default: show item
                    return true;
                });
            ...
            // Skip menu items that have no visible submenu
            if (m.submenu && (!sm || sm.length === 0)) return null;
...
// Export the wrapper component with auth
